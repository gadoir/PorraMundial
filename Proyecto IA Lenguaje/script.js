...
if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
        ...
        if (admins[user] && admins[user] === pass) {
            ...
            registerForm.closest('div').classList.add('hidden'); // Ocultar el contenedor de registro
            window.scrollTo({ top: adminPanel.offsetTop - 100, behavior: 'smooth' });
        } else {
            messageEl.classList.remove('hidden');
            messageEl.classList.add('bg-red-500/20', 'text-red-400');
            messageEl.innerText = 'Credenciales de administrador incorrectas.';
        }
    });
}
...
if (downloadSqlBtn) {
    downloadSqlBtn.addEventListener('click', () => {
        ...
        users.forEach(u => {
            ...
            sql += `INSERT INTO usuarios (username, email, password, fecha_registro) VALUES ('${cleanUser}', '${cleanEmail}', '${cleanPass}', '${u.fecha}');\n`;
        });
